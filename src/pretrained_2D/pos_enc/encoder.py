...
class MaskedGINDeepSigns_v3(nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers,
        ...
        self.rho = nn.Sequential(nn.Linear(out_channels, hidden_channels), nn.LeakyReLU(0.2),
                                 nn.Linear(hidden_channels, dim_pe), nn.LeakyReLU(0.2))
...
class SignNetNodeEncoder_v3(torch.nn.Module):
    ...
    def forward(self, batch):
        ...
        # SignNet
        pos_enc = self.sign_inv_net(pos_enc_pos, pos_enc_neg, batch.edge_index,
                                    empty_mask)  # (Num nodes) x (pos_enc_dim)
